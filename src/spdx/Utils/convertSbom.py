...
def convertCyclonedx(syft_json,project_name,output_file_cyclone,ExterDependencies):
    ...
    for externalDependency in ExterDependencies:
        ...
        component = Component(
            ...
            properties = [
                ...
                Property(name="syft::package:arch",value = arch),
                Property(name="syft:package:dscLink",value=dscLink)
    ...
    print('', '=' * 30, '', sep='\n')
#使用syft11的结果生成的spdx，针对deb软件包
def convertSpdx_Deb_syft11(syft_json, project_name, output_file, ExterDependencies):
    current_date = datetime.today().date()
    # 当前的时间
    current_time = datetime.now()
    # project就是scan_path，这里生成哈希码
